...
enum HWBlendFlags
{
	...
	BLEND_HW6    = 0x40,   // HW rendering is split in 3 passes
	BLEND_MIX1   = 0x80,   // Mix of hw and sw, do Cs*F or Cs*As in shader
	BLEND_MIX2   = 0x100,  // Mix of hw and sw, do Cs*(As + 1) or Cs*(F + 1) in shader
	BLEND_MIX3   = 0x200,  // Mix of hw and sw, do Cs*(1 - As) or Cs*(1 - F) in shader
	BLEND_ACCU   = 0x400,  // Allow to use a mix of SW and HW blending to keep the best of the 2 worlds
	BLEND_NO_REC = 0x800,  // Doesn't require sampling of the RT as a texture
	BLEND_A_MAX  = 0x1000, // Impossible blending uses coeff bigger than 1
};
