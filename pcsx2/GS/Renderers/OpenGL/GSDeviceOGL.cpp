...
void GSDeviceOGL::RenderHW(GSHWDrawConfig& config)
{
	...
	if (config.blend_multi_pass.enable_pass2)
	{
		if (config.blend.IsEffective(config.colormask))
		{
			OMSetBlendState(config.blend_multi_pass.blend_pass2.enable, s_gl_blend_factors[config.blend_multi_pass.blend_pass2.src_factor],
				s_gl_blend_factors[config.blend_multi_pass.blend_pass2.dst_factor], s_gl_blend_ops[config.blend_multi_pass.blend_pass2.op],
				s_gl_blend_factors[config.blend_multi_pass.blend_pass2.src_factor_alpha], s_gl_blend_factors[config.blend_multi_pass.blend_pass2.dst_factor_alpha],
				config.blend_multi_pass.blend_pass2.constant_enable, config.blend_multi_pass.blend_pass2.constant);
		}
		else
		{
			OMSetBlendState();
